...
import com.application.snackhub.databinding.FragmentBurgerBinding
...
class BurgerFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getMostRequestedBurgers()
    ...
    override fun onCreateView(
        ...
        binding = FragmentBurgerBinding.inflate(inflater)
        return binding.root
    }
    ...
    private fun productsPaging() {
        binding.scrollTea.setOnScrollChangeListener(NestedScrollView.OnScrollChangeListener { v, scrollX, scrollY, oldScrollX, oldScrollY ->
            if (v!!.getChildAt(0).bottom <= (v.height + scrollY)) {
                viewModel.getBurgers(productsAdapter.differ.currentList.size)
            }
        })
    }
        ...
        binding.rvHeader.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!recyclerView.canScrollHorizontally(1) && dx != 0)
                    viewModel.getMostRequestedBurgers(headerAdapter.differ.currentList.size)
    ...
    private fun observeProducts() {
        viewModel.burgers.observe(viewLifecycleOwner, Observer { response ->
    ...
    private fun observeHeader() {
        viewModel.mostRequestedBurgers.observe(viewLifecycleOwner, Observer { response ->
